...
main(int argc, char** argv)
{
	...
	string pointcloud_filename = "./test6.pcd";
...
//	pcl::io::loadPCDFile ("./sample8_bed.pcd", *cloud1);
	pcl::io::loadPCDFile ("./test7.pcd", *cloud1);
...
//	viewer->addSphere( source_pt, 0.05, 1, 1, 0);
	...
	HumanPose pose;
	pose.rHand  = vec_end_pt[3];
	pose.lHand  = vec_end_pt[4];
	pose.rAnkle = vec_end_pt[0];
	pose.lAnkle = vec_end_pt[2];
...
//	pose.lElbow.z = vecA(2)*pose.lowerArmLength + pose.lHand.z;
	...
	pose.lElbow.x = pose.rElbow.x + 0.03;
	pose.lElbow.y = pose.rElbow.y - 0.6;
	...
	pose.lShoulder.y = pose.rShoulder.y - 0.5;
	pose.lShoulder.z = pose.rShoulder.z - 0.05;
	...
	pcl::io::loadPCDFile ("./sample8_bed.pcd", *scene);
